...
*/
Node *tree_create_node(const void *key, const void *value){
	assert(key);
	assert(value);
...
*/
int count_value_in_subtree(const Map *obj, const Node *node, const void *value){
	assert(obj);
	assert(value);
...
*/
int tree_size_subtree(const Node *node){
